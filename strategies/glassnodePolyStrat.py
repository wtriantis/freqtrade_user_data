...
class glassnodePolyStrategy(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1d"
    minimal_roi = {"0": 1}
    stoploss = -1
    ...
    buy_exponent_mvrv = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_spss = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_nupl = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_realized = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_short_term_activity = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_short_term_ratio = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_miners = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_holder_spending = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_holder_mvrv = DecimalParameter(0.1, 2, default=1, space="buy")
    buy_exponent_exchange_volume = DecimalParameter(0.1, 2, default=1, space="buy")
    ...
    sell_exponent_mvrv = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_spss = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_nupl = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_realized = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_short_term_activity = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_short_term_ratio = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_miners = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_holder_spending = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_holder_mvrv = DecimalParameter(0.1, 2, default=1, space="sell")
    sell_exponent_exchange_volume = DecimalParameter(0.1, 2, default=1, space="sell")
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe['enter_long'] = (
            self.buy_weight_mvrv.value * np.power(dataframe.get('mvrv_Very Low Risk', 0), self.buy_exponent_mvrv.value) +
            self.buy_weight_spss.value * np.power(dataframe.get('spss_Very Low Risk', 0), self.buy_exponent_spss.value) +
            self.buy_weight_nupl.value * np.power(dataframe.get('nupl_Very Low Risk', 0), self.buy_exponent_nupl.value) +
            self.buy_weight_realized.value * np.power(dataframe.get('realized_Very Low Risk', 0), self.buy_exponent_realized.value) +
            self.buy_weight_short_term_activity.value * np.power(dataframe.get('short_term_activity_Very Low Risk', 0), self.buy_exponent_short_term_activity.value) +
            self.buy_weight_short_term_ratio.value * np.power(dataframe.get('short_term_ratio_Very Low Risk', 0), self.buy_exponent_short_term_ratio.value) +
            self.buy_weight_miners.value * np.power(dataframe.get('miners_Very Low Risk', 0), self.buy_exponent_miners.value) +
            self.buy_weight_holder_spending.value * np.power(dataframe.get('holder_spending_Very Low Risk', 0), self.buy_exponent_holder_spending.value) +
            self.buy_weight_holder_mvrv.value * np.power(dataframe.get('holder_mvrv_Very Low Risk', 0), self.buy_exponent_holder_mvrv.value) +
            self.buy_weight_exchange_volume.value * np.power(dataframe.get('exchange_volume_Very Low Risk', 0), self.buy_exponent_exchange_volume.value)
        )

        dataframe['enter_long'] = dataframe['enter_long'] > 0.5

        return dataframe

    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe['exit_long'] = (
            self.sell_weight_mvrv.value * np.power(dataframe.get('mvrv_Very High Risk', 0), self.sell_exponent_mvrv.value) +
            self.sell_weight_spss.value * np.power(dataframe.get('spss_Very High Risk', 0), self.sell_exponent_spss.value) +
            self.sell_weight_nupl.value * np.power(dataframe.get('nupl_Very High Risk', 0), self.sell_exponent_nupl.value) +
            self.sell_weight_realized.value * np.power(dataframe.get('realized_Very High Risk', 0), self.sell_exponent_realized.value) +
            self.sell_weight_short_term_activity.value * np.power(dataframe.get('short_term_activity_Very High Risk', 0), self.sell_exponent_short_term_activity.value) +
            self.sell_weight_short_term_ratio.value * np.power(dataframe.get('short_term_ratio_Very High Risk', 0), self.sell_exponent_short_term_ratio.value) +
            self.sell_weight_miners.value * np.power(dataframe.get('miners_Very High Risk', 0), self.sell_exponent_miners.value) +
            self.sell_weight_holder_spending.value * np.power(dataframe.get('holder_spending_Very High Risk', 0), self.sell_exponent_holder_spending.value) +
            self.sell_weight_holder_mvrv.value * np.power(dataframe.get('holder_mvrv_Very High Risk', 0), self.sell_exponent_holder_mvrv.value) +
            self.sell_weight_exchange_volume.value * np.power(dataframe.get('exchange_volume_Very High Risk', 0), self.sell_exponent_exchange_volume.value)
        )

        dataframe['exit_long'] = dataframe['exit_long'] > 0.5
